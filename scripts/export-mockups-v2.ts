...
import { LOCAL_BUSINESS_MOCKUPS_V2 } from "../lib/mockups-v2-data";
...
async function exportMockupsV2() {
  const rootDir = getWorkspaceRoot();
  const outputDir = path.join(rootDir, "public", "mockups-v2");
  await mkdir(outputDir, { recursive: true });

  const explicitBaseUrl = process.env.MOCKUP_V2_EXPORT_BASE_URL?.trim();
  const port = Number(process.env.MOCKUP_V2_EXPORT_PORT ?? DEFAULT_PORT);
  ...
  try {
    ...
    for (const mockup of LOCAL_BUSINESS_MOCKUPS_V2) {
      const url = `${baseUrl}/mockups-v2/${mockup.slug}`;
      ...
      if (!boundingBox) {
        throw new Error(`Could not measure V2 mockup canvas for ${mockup.slug}`);
      }
